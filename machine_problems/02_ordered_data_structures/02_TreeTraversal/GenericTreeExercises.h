...
static void treeFactory(GenericTree<int> &tree)
{
    ...
    auto nodeC = nodeA->addChild(8);
    nodeA->addChild(15);
    auto nodeE = nodeC->addChild(16);
    nodeC->addChild(23);
    nodeE->addChild(42);
...
static void treeFactoryTest()
{
    ...
    std::cout << "The output should match what you see in the code comments"
              << std::endl
              << std::endl;
...
//
// template <typename T>
// int countNullChildrenRecursive(typename GenericTree<T>::TreeNode*
// subtreeRoot) { ... }
//
...
int countNullChildrenRecursive(N *subtreeRoot)
{
    ...
    //  sure that it was not null first. We already handled that base case
    //  above.
...
int countNullChildrenIterative(N *subtreeRoot)
{
    ...
    while (!nodesToExplore.empty()) {
        ...
        // Make a copy of the top pointer on the stack, then pop it to decrease
        // the stack
        ...
        if (!topNode) {
            ...
            // Just increment the null counter, then "continue" to jump back to
            // the top of the loop.
            nullChildrenSum++;
            continue;
        }
...
std::vector<T> traverseLevels(GenericTree<T> &tree)
{
    ...
    // Begin by pushing our subtree root pointer onto the queue
    nodesToExplore.push(rootNodePtr);

    // Loop while there are still nodes to explore
    while (!nodesToExplore.empty()) {

        // Make a copy of the top pointer on the queue, then pop it to decrease
        // the queue
