...
public:
    ...
    public:
        ...
        // Add a rightmost child to this node storing a copy of the provided
        // data. Returns a pointer to the new child node.
        ...
        // There is a special syntax for disabling certain constructors
        // entirely. This inhibits default versions from being generated by the
        // compiler. We'll do this here for simplicity, and to prevent you from
        // attempting the assignment in a way other than intended.
...
public:
    ...
    void clear()
    {
        ...
        if (rootNodePtr) {
            throw std::runtime_error("clear() detected that deleteSubtree() "
                                     "had not reset rootNodePtr");
        }
    }
    ...
    // Print the tree to the output stream (for example, std::cout) in a
...
typename GenericTree<T>::TreeNode *GenericTree<T>::createRoot(const T &rootData)
{
    ...
    // then the root node already exists, and it's an error to try to recreate
    // it.
    if (nullptr != rootNodePtr) {
        // We don't always need to import <string> just to reuse a short error
        // message. We can store short strings as constant arrays of char type
        // at compile time. (A constexpr is similar to const, but must be
        // entirely evaluated at compile time.)
...
template <typename T>
typename GenericTree<T>::TreeNode *
GenericTree<T>::TreeNode::addChild(const T &childData)
{
...
void GenericTree<T>::deleteSubtree(TreeNode *targetRoot)
{
    ...
    // class instance that's calling the function.
    {
        ...
        while (walkBack->parentPtr) {
            // Walk back from the targeted node to its ultimate parent, the
            // root. (The root has no parent, so the walk ends there.)
        ...
        if (walkBack != rootNodePtr) {
            throw std::runtime_error(
                "Tried to delete a node from a different tree");
        }
    }
    ...
    if (targetRoot->parentPtr) {
        ...
        // targetRoot->parentPtr->childrenPtrs that we are iterating over,
        // instead of acting on temporary copies.) If the child is found under
        // its parent as expected, overwrite it in-place with nullptr.
        for (auto &currentChildPtr : targetRoot->parentPtr->childrenPtrs) {
            if (currentChildPtr == targetRoot) {
                ...
                targetWasFound = true;
                // Stop looping early. The "break" statement exits the current
                // "for" loop and moves on to the next statement outside.
                break;
            }
        }
    ...
    while (!nodesToExplore.empty()) {
        ...
        // Now that we've retrieved the top pointer, we can pop it from the
        // explore stack.
        nodesToExplore.pop();
        ...
        for (auto childPtr : curNode->childrenPtrs) {
            // Push a copy of the child pointer onto the stack of children to
            // explore
...
void GenericTree<T>::compress()
{
    ...
    // long as they have a robust base case to handle "exploring" a null
    // pointer, but this time, whenever we explore a new node, we're going to
    // make a compressed copy of its vector of children pointers that has all
    // the null pointers removed.
    ...
    while (!nodesToExplore.empty()) {
        ...
        // Make a copy of the front pointer on the queue, then pop it to
        // decrease the queue
        TreeNode *frontNode = nodesToExplore.front();
        nodesToExplore.pop();

        if (!frontNode) {
            // The front node pointer should not be null, because we're
            // designing this function so that no null pointers should ever get
            // pushed onto the exploration queue.
            throw std::runtime_error(
        ...
        for (auto childPtr : frontNode->childrenPtrs) {
            if (childPtr) {
                ...
                // Also put this child pointer onto the end of the exploration
                // queue.
                nodesToExplore.push(childPtr);
            }
        }

        // Here's a little trick: the std::vector::swap() function lets us
        // replace the node's actual children pointer vector with the new one,
        // even though the compressed one is a local variable here. The standard
...
std::ostream &GenericTree<T>::Print(std::ostream &os) const
{
    ...
    // Here, we're using a std::vector for each node's collection of
    // childrenPtrs. The std::vector class allows index-based lookups like an
    // array using [], which is potentially unsafe if the index given is out of
    // bounds, or alternatively using .at(), which checks that the index is
    // legal and throws an exception otherwise. So, being careful, we could
    // write such a loop like this:
    ...
    while (!nodesToExplore.empty()) {
        ...
        if (showDebugMessages) {
            ...
            for (int row = 1; row <= LAST_ROW; row++) {
                // Iterate forward through the margin display flags to fill in
                // the margin.
                for (auto stemIt = curMargin.begin(); stemIt != curMargin.end();
                    ...
                    if (isLastCol) {
                        if (LAST_ROW == row) {
                            // The stem before the data item should always be
                            // "|_ " in effect:
                            os << stemSymbol << "_ ";
                        }
                        else if (showStem) {
                            ...
                            // Don't bother displaying trailing spaces before
                            // the newline
                            os << std::endl;
                        }
                    }
        ...
        if (curNode && curNode->childrenPtrs.size() > 0) {
            ...
            // Now, we iterate over childrenPtrs in reverse order. The iterator
            // "it" will begin at the "reverse beginning", and we'll iterate it
            // as long as it's not the "reverse ending". When we do it++, we do
            // iterate in the reverse direction correctly.
            for (auto it = curNode->childrenPtrs.rbegin();
                 it != curNode->childrenPtrs.rend(); it++) {

                // The iterator points to one child pointer. So here, the
                // iterator is like a pointer-to-pointer. By dereferencing the
                // iterator, we get a child pointer value. We can save a
                // temporary copy of that child pointer to a local variable.
                const TreeNode *childPtr = *it;

                // Now, push the child pointer onto the stack of children to
                // explore.
                nodesToExplore.push(childPtr);
                ...
                // Prepare a working copy of the margin for the node we're
                // pushing.
                ...
                // But for the trailing margin, we need to leave the rightmost
                // child with a blank trailing in the margin, because it's
                // displayed lowest.
                ...
                if (curNode->childrenPtrs.rbegin() == it) {
                    ...
                    // Other children leave a vertical stem symbol trailing in
                    // the margin.
