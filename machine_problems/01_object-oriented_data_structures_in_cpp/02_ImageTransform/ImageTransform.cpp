...
PNG grayscale(PNG image)
{
    ...
    for (unsigned x = 0; x < image.width(); x++) {
        for (unsigned y = 0; y < image.height(); y++) {
            ...
            // `pixel` is a reference to the memory stored inside of the PNG
...
PNG createSpotlight(PNG image, int centerX, int centerY)
{
    ...
    for (unsigned x = 0; x < image.width(); x++) {
        for (unsigned y = 0; y < image.height(); y++) {
            ...
            lfactor = 1.0 - (std::fmin(distR * 0.005, 0.8));
            pixel.l = pixel.l * lfactor;
        }
    }
    return image;
}
...
PNG illinify(PNG image)
{
    ...
    for (unsigned x = 0; x < image.width(); x++) {
        for (unsigned y = 0; y < image.height(); y++) {
            HSLAPixel &pixel = image.getPixel(x, y);
            distO = std::fmin(std::abs(pixel.h - orange),
                              360.0 - std::abs(pixel.h - orange));
            distB = std::fmin(std::abs(pixel.h - blue),
                              360.0 - std::abs(pixel.h - blue));
            if (distO < distB) {
                pixel.h = orange;
            }
            else {
...
PNG watermark(PNG firstImage, PNG secondImage)
{
    ...
    for (unsigned x = 0; x < width; x++) {
        for (unsigned y = 0; y < height; y++) {
            ...
            if (pixel2.l == 1.0) {
                pixel1.l = std::fmin(pixel1.l + 0.2, 1.0);
            }
        }
    }
    return firstImage;
