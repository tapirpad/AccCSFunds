/**
 * @file PNG.cpp
 * Implementation of a simple PNG class using HSLAPixels and the lodepng PNG
 * library.
 *
...
bool PNG::operator!=(PNG const &other) const { return !(*this == other); }

HSLAPixel &PNG::getPixel(unsigned int x, unsigned int y) const
{
    if (width_ == 0 || height_ == 0) {
        cerr << "ERROR: Call to uiuc::PNG::getPixel() made on an image with no "
                "pixels."
             << endl;
        assert(width_ > 0);
        assert(height_ > 0);
    ...
    if (x >= width_) {
        cerr << "WARNING: Call to uiuc::PNG::getPixel(" << x << "," << y
             << ") tries to access x=" << x
             << ", which is outside of the image (image width: " << width_
             << ")." << endl;
    ...
    if (y >= height_) {
        cerr << "WARNING: Call to uiuc::PNG::getPixel(" << x << "," << y
             << ") tries to access y=" << y
             << ", which is outside of the image (image height: " << height_
             << ")." << endl;
...
{
    os << "PNG(w=" << png.width() << ", h=" << png.height()
       << ", hash=" << std::hex << png.computeHash() << std::dec << ")";
    return os;
}
