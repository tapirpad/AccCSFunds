...
void informalTests()
{
    ...
    std::cout << "Running a few informal tests. Feel free to study these or "
                 "edit them."
              << std::endl;
    std::cout
...
void wordcountTests()
{
    ...
    std::cout << "======= Testing makeWordCounts =======" << std::endl
              << std::endl;
    ...
    std::cout << "Unique string occurrences: " << wordcount_map.size()
              << std::endl;
    StringIntPairVec sorted_wordcounts = sortWordCounts(wordcount_map);

    std::cout << "Sorted size: " << sorted_wordcounts.size() << std::endl
              << std::endl;
    ...
    StringIntPairVec bottom_wordcounts =
        getBottomWordCounts(sorted_wordcounts, 20);
    ...
    if (wordcount_map.bucket_count() < 1) {
        ...
        std::cout << "Reported load factor: " << wordcount_map.load_factor()
                  << std::endl;
        double load_factor =
            (double)wordcount_map.size() / wordcount_map.bucket_count();
        std::cout << "Calculated load factor: " << load_factor << std::endl
                  << std::endl;
    }

    if (!wordcount_map.count("bandersnatch") ||
        wordcount_map.at("bandersnatch") != 3) {
        std::cout << "Looks like makeWordCounts isn't implemented "
                     "yet.\nSkipping lookupWithFallback tests based upon it."
                  << std::endl
                  << std::endl;
        return;
    }
    ...
    std::cout << "Assuming makeWordCounts is working, here are some tests of "
                 "lookupWithFallback:"
              << std::endl;
    int bandersnatch_count =
        lookupWithFallback(wordcount_map, "bandersnatch", 0);
    std::cout << "Looking up \"bandersnatch\" with fallback of 0. Result "
                 "(expecting 3): "
              << bandersnatch_count << std::endl;
    if (bandersnatch_count != 3)
        errorReaction("Should have found \"bandersnatch\" with a count of 3.");
    else
        std::cout << "Good!" << std::endl;
    std::cout << "Checking that wordcount_map was not modified by the lookup:"
              << std::endl;
    if (wordcount_map_backup == wordcount_map)
        std::cout << "Good, wordcount_map wasn't changed by the lookup."
                  << std::endl;
    else
        errorReaction("The lookup operation changed wordcount_map somehow.");
    std::cout << std::endl;
    ...
    std::cout
        ...
        << std::endl;
    std::cout << "Verifying that the original wordcount_map doesn't contain "
                 "\"cheshire\":"
              << std::endl;
    if (wordcount_map.count("cheshire"))
        errorReaction(
            "Found \"cheshire\" in the map. That shouldn't have happened.");
    else
        std::cout << "Good, \"cheshire\" was not found, as expected."
                  << std::endl;
    int cheshire_count = lookupWithFallback(wordcount_map, "cheshire", -1);
    std::cout << "Looking up \"cheshire\" with fallback of -1. Result "
                 "(expecting -1): "
              << cheshire_count << std::endl;
    if (cheshire_count != -1)
        errorReaction("Should not have found \"cheshire\" in map, and should "
                      "have reported -1 as fallback.");
    else
        std::cout << "Good!" << std::endl;
    std::cout << "Checking that wordcount_map was not modified by the lookup:"
              << std::endl;
...
void lookupWithFallbackTests()
{
...

    {
        ...
        if (bandersnatch_count != 3)
            errorReaction(
                "Should have found \"bandersnatch\" with a count of 3.");
        else
            std::cout << "Good!" << std::endl;
        std::cout
        ...
        if (wordcount_map_backup == wordcount_map)
            ...
            errorReaction(
                "The lookup operation changed wordcount_map somehow.");
        std::cout << std::endl;
    }

    {
        ...
        std::cout << "Looking up \"cheshire\" with fallback of -1. Result "
                  ...
                  << cheshire_count << std::endl;
        if (cheshire_count != -1)
            errorReaction("Should hould have reported -1 as fallback\n after "
                          "not finding \"cheshire\" in the map.");
        else
            std::cout << "Good!" << std::endl;
        std::cout
            << "Checking that wordcount_map was not modified by the lookup:"
...
void hashingDemo()
{
    ...
    {
        std::cout << "======= Demonstrating the default string hasher ======="
                  << std::endl
                  << std::endl;

        std::hash<std::string> stringHasher;

        std::cout << "Notice that changing the string by a single character "
                     "will significantly\n change its hash:"
                  << std::endl
                  << std::endl;
...

        {
            std::string str = "jabberwocky";
            std::cout << "String: \"" << str << "\"" << std::endl;
            // Here is an example of more advanced C++ syntax you might see in
            // the future. In the follow line, the first apperance of "()"
            // syntax only constructs a temporary, anonymous instance of
            // std::hash<std::string>. Then, the object's operator() member is
            // immediately called on str.
    ...
    {
        std::cout << "======= Demonstrating our IntPair hasher ======="
                  << std::endl
                  << std::endl;

        std::cout << "Note:\n You don't need to manually invoke our custom "
                     "hasher to do this assignment."
                  << std::endl
                  << " Instances of std::unordered_map<IntPair, int> will "
...
void palindromeTests()
{
    ...
    // Small example: Brute force is pretty fast, but memoization gives a
    // noticeable speedup.
    const std::string str_small = "abbbcdeeeefgABCBAz";
    // Medium example: Brute force takes several seconds. Memoization is very
    // fast.
    const std::string str_medium = "abbbcdeeeefgABCBAabcdefgh";
    // Large example: Brute force will probably time out. Memoization is still
    // very fast.
    ...
    // For large problem sizes, the memoized version can be thousands of times
    // faster.

    {
        std::cout << "======= Finding the longest palindrome substring (brute "
                     "force) ======="
                  << std::endl
                  << std::endl;
        std::cout << "Note: This may take a few seconds. The duration scales "
        ...
        try {
            pal_result = longestPalindromeLength(str, 0, str.length() - 1,
                                                 start_time, max_duration);
            stop_time = getTimeNow();
            intended_pal_result = pal_result;
            have_intended_result = true;
            std::cout << "Length of longest palindrome substring: "
                      << pal_result << std::endl;
        }
        catch (TooSlowException &e) {
            stop_time = getTimeNow();
            std::cout << "(Timeout!) Brute force is taking too long on this "
                         "example. Giving up."
                      << std::endl;
        }
    ...
    {
        std::cout << "======= Finding the longest palindrome substring "
                     "(memoized) ======="
                  << std::endl
                  << std::endl;
        std::cout << "Note: If you haven't finished correctly implementing "
                     "this function yet,"
                  << std::endl
                  << " the provided starter code may take even longer than "
                     "brute force and time out."
                  << std::endl
                  << " That could also happen if you have a bug or infinite "
                     "recursion."
                  << std::endl
                  << std::endl;
        std::cout << "String: \"" << str << "\"" << std::endl;
        ...
        try {
            pal_result = memoizedLongestPalindromeLength(
                memo, str, 0, str.length() - 1, start_time, max_duration);
        }
        catch (TooSlowException &e) {
            timed_out = true;
        }
        ...
        if (timed_out) {
            std::cout << "(Timeout!) Elapsed time (ms): " << memoized_time
                      << std::endl;
            ...
            if (have_intended_result) {
                if (pal_result != intended_pal_result)
                    errorReaction("Your memoized palindrome function returned "
                                  "the wrong result.");
                else
                    std::cout << " (Correct!)" << std::endl;
            }
            std::cout << "Calculation time (ms): " << memoized_time
                      << std::endl;
            double speedup = slow_time / memoized_time;
            std::cout << "Speedup over brute force: " << speedup
                      << " times faster" << std::endl;
            if (speedup < 2.0)
                errorReaction("Should have been at least 2 times faster.\n (A "
                              "correct memoized solution will probably even be "
                              "MUCH faster than that.)");
            else
                std::cout << " (Good!)" << std::endl;

            std::string found_palindrome = reconstructPalindrome(memo, str);
            std::cout << "Palindrome found (based on memoization): \""
                      << found_palindrome << "\"" << std::endl;
            if (found_palindrome != correct_palindrome_substring)
                errorReaction(
                    "The substring found is incorrect, so the memoization "
