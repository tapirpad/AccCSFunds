...
StringVec loadBookStrings(unsigned int min_word_length)
{
    ...
    // We'll just handle the input file as plain text. (It is actually encoded
    // as UTF-8, which is partially compatible as "plain text," but some special
    // characters or punctuation in the file cannot be represented as an
    // ordinary "char" in C++. In practice, parsing text robustly can be much
    // more complicated than this, but it's a topic for another course.)

    // Open the input file for reading with an ifstream, input file stream
    // object. If it can't be opened, throw an exception.
    ...
    {
        // Read lines of the input one at a time, as a temporary copy string
        // "line".
        std::string line;
        while (std::getline(instream, line)) {
            // While we're still getting lines from the file... (This "while"
            // block doesn't execute at all if no lines could be retrieved, such
            // as if the file turned out to be blank, if we named the wrong file
            // locally.)
            ...
            // std::string::npos, then we DID find start_text within this line,
            // and so we break out of the loop, because we're done
            // fast-forwarding.
    ...
    while (stillReading && std::getline(instream, line)) {
        ...
        // This uses regex ("regular expressions") to find and replace
        // troublesome string patterns. That makes it easier to parse below in a
        // simple way.
            ...
            for (char c : line) {
                ...
                if (std::isalpha(c)) {
                    ...
                    if (prevChar == '\'') {
                        ...
                        // Then the first letter has already been recorded, and
                        // now we'll put down the apostrophe, and then the
                        // following letter.
                        trimmed_word += "'";
                    }
                    else if (prevChar == '-') {
                        // Likewise, we'll allow [letter][dash][letter] to
                        // include the dash in the word.
                        trimmed_word += "-";
                    }
                    ...
                    // Only note an apostrophe as a previous character if it
                    // follows a letter. Don't add it to the recorded word yet.
                            ...
                            double maxDuration)
{
    ...
    if (rightLimit < 0) {
        ...
        throw std::runtime_error(
            "rightLimit negative, but it's not the base case");
    }
    ...
    if (false) {
        ...
        std::cout << "Considering substring: " << str.substr(leftLimit, range)
                  << std::endl;
        std::cout << " because l/r limits are: " << leftLimit << " "
                  << rightLimit << std::endl;
    }

    // Some examples may take an absurdly long time to calculate with brute
    // force.
    ...
    if (str.at(leftLimit) == str.at(rightLimit)) {
        ...
        int middleSubproblemResult = longestPalindromeLength(
            str, newLeft, newRight, startTime, maxDuration);

        // (Base case note: Suppose that str had length 2, so after moving the
        // indices,
        //  now newLeft > newRight. Because we handled the crossing case
        //  already, in that situation, middleSubproblemResult correctly gets
        //  value 0.)
