...
StringIntMap makeWordCounts(const StringVec &words)
{
    ...
    for (unsigned long i = 0; i < words.size(); i++) {
        if (wordcount_map.count(words[i])) {
                                    ...
                                    timeUnit startTime, double maxDuration)
{
    ...
    // Check validity of indices for debugging. The indices shouldn't be
    // negative unless it's the special base case where they cross during
    // recursion. We handle that case further below.
    if (leftLimit < 0 && leftLimit <= rightLimit) {
        throw std::runtime_error(
            "leftLimit negative, but it's not the base case");
    }
    ...
    // The count() function of unordered_map tells us if the key is already in
    // the map. It returns 1 if found and otherwise 0. (These values convert to
    // true and false.) (It's important to not just use memo[pairKey] in this
    // check, because that will
    ...
    if (memo.count(pairKey)) {
        ...
        // new in this case. So, we also won't store anything new in the table
        // in this case, only return what's already stored at this key in the
        // map.
    ...
    if (str.at(leftLimit) == str.at(rightLimit)) {
        ...
        if (middleSubproblemResult == middleMaxLength) {
            ...
            // This result, which we should memoize, is for the range between
            // the original leftLimit and rightLimit. (It's not for the inner
            // range between newLeft and newRight. The recursive call already
            // memoized that.)
