...
struct hash<IntPair> {
    ...
    std::size_t operator()(const IntPair &p) const
    {
        ...
        std::string uniqueIntPairString =
...
static inline std::ostream &operator<<(std::ostream &os, const IntPair &point)
{
    ...
    if (point.first < 10 && point.first >= 0)
        os << " "; // optional padding space
    os << "(";
    os << point.first;
    os << ",";
    os << point.second;
    os << ")";
    if (point.second < 10 && point.second >= 0)
        os << " "; // optional padding space
    return os;
}
...
struct hash<IntPairPair> {
    ...
    std::size_t operator()(const IntPairPair &twoPairs) const
    {
        ...
        // edge in this project, but there are certain lines of code in
        // GridGraph.h that manually ensure that ((1,2),(2,2)) and ((2,2),(1,2))
        // will be considered equal when inserting to std::unordered_set. We
        // could take this further by making a new derived class based on
        // IntPairPair, explicitly intended for representing undirected edges
        // only, and then defining new equivalence operators and hashing support
        // for it. But this time, we will just let IntPairPair be generic, and
        // manually take care of the edge flipping in GridGraph.h.
        uniqueString =
