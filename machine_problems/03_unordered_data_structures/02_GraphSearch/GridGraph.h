...
public:
    ...
    // The values are of type NeighborSet. Thus, each key is associated with a
    // set of coordinates that are the adjacent points (those connected by an
    // edge).
    ...
    // verbose text output will be used instead. This can be set to false in
    // your main program setup to also prevent that output, in case there are
    // very many edges and points to list.
    ...
    void insertEdge(const IntPair &p1, const IntPair &p2)
    {
        ...
        // adjacencyMap[p1] creates an entry with key p1 in the map if
        // necessary, or otherwise it looks up the existing one, and then it
        // returns a reference to p1's associated NeighborSet. (If it was just
        // created now by this usage of [], then that set will be empty.) Then,
        // .insert(p2) calls the NeighborSet's insert() function to insert p2.
    ...
    bool hasEdge(const IntPair &p1, const IntPair &p2) const
    {
        ...
        if (directions == 0) {
            ...
            // This means one endpoint refers to the other, but not the other
            // way around. Both ends should be recorded as adjacent to each
            // other. Most likely this means there is either a bug in
            // insertEdge, or the programmer tried to insert an edge manually
            // and made a mistake, or an endpoint was deleted but maintenance
            // was not performed on its neighboring points' adjacency lists.
    ...
    // current instance of the class, so "*this" is the current GridGraph
    // itself.
