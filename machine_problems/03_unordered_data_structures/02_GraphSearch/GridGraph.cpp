...
std::ostream &GridGraph::plot(std::ostream &os) const
{
    ...
    // Identify the min and max row and col in use by any point (or 0 for an
    // empty graph).
    int minRow = 0;
    int minCol = 0;
    int maxRow = 0;
    ...
    // on the left side of the terminal output. (This part is a bit
    // complicated.)
    for (int row = minRow; row <= maxRow; row++) {
        // A string stream lets us do stream operations with "<<" to prepare a
        // string. We'll do this to prepare the decorations in the line below
        // the current line we are printing.
        std::stringstream understream;
        for (int col = minCol; col <= maxCol; col++) {
            // pos is the position at {row,col} in the grid, which may not
            // actually be a point in the graph
            IntPair pos = {row, col};
            // If this point is in the graph, put a point marker.
            if (hasPoint(pos)) {
                // Output the point (expected to be padded to 7 characters): "
                // (1,2) ", "(12,34)"
                os << pos;
            }
            ...
            // If there's no edge to the right, and this isn't the rightmost
            // column, then put padding spaces
            else if (col < maxCol) {
                os << "    ";
            }
            ...
            if (row < maxRow) {
                ...
                // If there's an edge beneath this point, then draw a vertical
                // line
        ...
        // We're done processing this row, so cap this line of os with an
        // endline
        os << std::endl;
...
std::ostream &GridGraph::printDetails(std::ostream &os) const
{
    ...
    for (const auto &kv : adjacencyMap) {
        ...
        // to be careful to ensure unique representation: For any points A and
        // B, the edge (A,B) is the same as the edge (B,A). To keep an edge key
        // consistent regardless of which direction it is viewed from, an
        // ordering is imposed on the endpoints using the "<" operator. Note
        // that we can compare two IntPair with "<" because STL gives us an
        // implementation of "<" for std::pair<int,int> by default.
        else {
            for (const auto &p2 : p1_neighbors) {
                ...
                if (p1 < p2) {
                    ...
                    // If the edge key looks "backwards," then flip it for
                    // uniformity. That is, the same undirected edge (A,B) and
                    // (B,A) will always be recorded as (A,B).
