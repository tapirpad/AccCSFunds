...
std::list<IntPair> graphBFS(const IntPair &start, const IntPair &goal,
                            const GridGraph &graph)
{
    ...
    // maxDist failsafe:
    //
    // We'll hard-code a maximum distance for any shortest path through the
    // graph that we will consider before giving up. This isn't strictly
    // necessary for BFS to be correct; when a path can be found from start to
    // goal, we'll find the shortest such path and we won't alwaysneed to visit
    // all of the vertices before that happens anyway. But suppose the goal is
    // unreachable from start; then if the graph has a very large number of
    // vertices, an unrestricted BFS algorithm would have to explore all the
    // reachable vertices before giving up anyway, which could require a lot of
    // time and memory if the graph is huge. To avoid this issue, we can
    // hard-code the maximum number of steps we will allow before giving up. Of
    // course, we must ensure that it's a reasonable number. We might be able to
    // estimate the maximum number of steps based on the number of edges in the
    // graph, for example. In the case of puzzleBFS later in this assignment,
    // there is a proven upper bound on the number of steps needed.
    //
    ...
    // (first in, first out), not a priority queue. When we say "explore" a
    // vertex in this assignment, we mean to dequeue it and loop over its
    // adjacent vertices.
    std::queue<IntPair> exploreQ;

    // Map that associates a given point with its predecessor (the neighbor that
    // discovered it first) This will allow us to reconstruct the path once the
    // goal is found.
    std::unordered_map<IntPair, IntPair> pred;
    // Example usage: "pred[B] = A" records A as the predecessor of B, meaning
    // that A discovered B.

    // Map that associates a given point with its shortest-path distance from
    // the starting point. This lets us see if we've taken too many steps (see
    // the above comment about maxDist). The simplest implementations of BFS
    // don't need a distance record like this, but it can be useful information,
    // and other graph search algorithms like Dijkstra's algorithm do need to
    // use such a record. Remember: with std::unordered_map, the first time we
    // look up a key with [], it will be
    //  inserted if it doesn't exist yet. Also, its initial int value will be 0
    //  by default.
    ...
    // (For further optimization, we could just use pred to query whether a
    // vertex
    ...
    //  It would default-initialize dist[start] to 0 later when we referred to
    //  it. However, this is more clear.)
    dist[start] = 0;

    // We begin at the start point, so mark it discovered by putting a copy of
    // it in the visited set.
    visitedSet.insert(start);

    // We will begin searching from the start point. Push a copy of it into the
    // exploration queue.
    exploreQ.push(start);

    // Below, a "flag" just means a variable for controlling a loop condition.

    // foundGoal is a flag for controlling the loop below. It's initialized as
    // false, unless we start at the goal, in which case it's initialized as
    // true.
    bool foundGoal = (start == goal);

    // tooManySteps is a flag that will help us break out of the loop if we
    // detect that we've already taken too many steps and probably need to give
    // up searching.
    bool tooManySteps = false;

    // The main search loop --------------------------------------------------

    // While the exploration queue isn't empty yet, there are still discovered
    // points to explore. Also, we would want to stop looping if foundGoal or
    // tooManySteps became true.
    while (!exploreQ.empty() && !foundGoal && !tooManySteps) {
        ...
        // Error prevention: Check whether this vertex has ever been dequeued in
        // the past. (Checking for vertices that are dequeued more than once
        // isn't strictly considered
        //  part of the BFS algorithm, because in the loop below, you are meant
        //  to check for previously-visited vertices while considering the
        //  adjacent vertices. We do this here for additional sanity checking,
        //  in case you have made any mistakes that would cause an infinite
        //  loop.)
        bool curPointWasPreviouslyDequeued = dequeuedSet.count(curPoint);
        if (curPointWasPreviouslyDequeued) {
            std::cout << "graphBFS ERROR: Dequeued a vertex that had already "
                         "been dequeued before."
                      << std::endl
                      << " If you're using visitedSet correctly, then no "
                         "vertex should ever be added"
                      << std::endl
                      << " to the explore qeueue more than once. [Returning an "
                         "empty path now.]"
                      << std::endl
                      << std::endl;
            ...
            // We'll record that this vertex has been dequeued by adding a copy
            // of it to the set.
        ...
        // curPoint. Get a copy of the set of neighbors we're going to loop
        // over.
        ...
        for (auto neighbor : neighbors) {
            ...
            // If this adjacent vertex has NOT been visited before, we will
            // visit it now. If it HAS been visited before, we do nothing and
            // continue to loop. This way, we avoid enqueueing the same vertex
            // more than once.
            if (!neighborWasAlreadyVisited) {

                // ================================================================

                // Record that the curPoint is the predecessor of the neighbor
                // point, since curPoint has just led to the discovery of this
                ...
                // The shortest-path distance to this neighbor is the
                // shortest-path distance to curPoint, plus one. (We know that
                // dist[curPoint] has already been initialized because we
                // previously reached curPoint from another point, and it was
                // assigned a value at that time, or else curPoint is the start
                // point that we explicitly initialized at the beginning.)
                dist[neighbor] = dist[curPoint] + 1;
                if (dist[neighbor] > maxDist) {
                    // If the shortest path to this neighbor is a further
                    // distance than we are allowed to explore, then flag that
                    // we have taken too many steps, and break out of the
                    // nearest loop (the for loop). After that, the tooManySteps
                    // flag will cause the while loop to stop also.

                    // You may ask: How do we know to give up here? What if
                    // there are other paths we can still explore through the
                    // exploration queue, that are shorter? We know there are
                    // not, because in BFS, we identify all the reachable nodes
                    // of distance 1, and only after that, all the reachable
                    // nodes with distance 2, etc. So right now, if we need "too
                    // many" steps to reach this discovered neighbor, we'll also
                    // need "too many" steps to reach any other node later. This
                    // is the same reasoning that allows us to conclude we have
                    // found the shortest path, when we finally do reach the
                    // goal.
                    tooManySteps = true;
                    break;
                }

                // If we haven't taken too many steps yet, and if the neighbor
                // is the goal, then flag that we have found the goal, and break
                // out of the "for" loop. The foundGoal flag will then cause the
                // "while" loop to stop, also.
                if (neighbor == goal) {
                    foundGoal = true;
                    break;
                }
    ...
    // If we took too many steps, we issue an error message and return an empty
    // list.
    if (tooManySteps) {
        std::cout
            << "graphBFS warning: Could not reach goal within the maximum "
               "allowed steps.\n (This may be expected if no path exists.)"
            << std::endl
            << std::endl;
        return std::list<IntPair>();
    }

    // If we never found the goal even after exploring the entire reachable
    // graph, then issue an error message and return an empty list.
    if (!foundGoal) {
        std::cout << "graphBFS warning: Could not reach goal. (This may be "
                     "expected\n if no path exists.)"
                  << std::endl
                  << std::endl;
        return std::list<IntPair>();
    ...
    // Otherwise, we must have found a path from start to goal, and it is as
    // short as possible. (If there exist multiple shortest paths from start to
    // goal that all have the same length,
    //  we have found ONE of these shortest paths.)

    // Make a new, empty list of IntPairs in the stack memory at function scope.
    // This will represent the path from start to goal. We'll return it by value
    // at the end.
    std::list<IntPair> path;
    // We will walk back from the goal to the start using the predecessor
    // records.
    auto cur = goal;
    // Push the currently-walked vertex onto the FRONT of the list, so that as
    // we walk back from goal to start, the created list will be in order from
    // start to goal.
    path.push_front(cur);
    // Make sure that there is a predecessor recorded for cur, and then while
    // the predecessor of cur is not recorded as itself, keep looping. (The
    // start vertex recorded itself as its predecessor, so then we know to
    // stop.)
    while (pred.count(cur) && pred[cur] != cur) {

        // It's good to note here that the "&&" operator has "short circuiting"
        // behavior, which means that in "A && B", if A is false, then B is
        // never evaluated at all. The whole expression is immediately
        // determined to be false. This way, the "A" part can be used as a
        // safeguard on the "B" part, if "B" could cause problems.

        // Push a copy of the predecessor onto the front of the list, as we
...
std::list<PuzzleState> puzzleBFS(const PuzzleState &start,
                                 const PuzzleState &goal)
{
    ...
    while (!exploreQ.empty() && !foundGoal && !tooManySteps) {
        ...
        if (curPointWasPreviouslyDequeued) {
            std::cout << "puzzleBFS ERROR: Dequeued a vertex that had already "
                         "been dequeued before."
                      << std::endl
                      << " If you're using visitedSet correctly, then no "
                         "vertex should ever be added"
        ...
        // =====================================================================
        // We'll need to loop over the neighbors that are the points adjacent to
        // curState. We need a collection of neighbors we're going to loop over.
        ...
        for (auto neighbor : neighbors) {
            ...
            if (!neighborWasAlreadyVisited) {
                ...
                // Record that the curState is the predecessor of the neighbor
                // point, since curState has just led to the discovery of this
                // neighbor for the first time.
    ...
    if (tooManySteps) {
        std::cout
            << "puzzleBFS warning: Could not reach goal within the maximum "
               "allowed steps.\n (This may be expected if no path exists.)"
            << std::endl
            << std::endl;
        return std::list<PuzzleState>();
    ...
    if (!foundGoal) {
        std::cout << "puzzleBFS warning: Could not reach goal. (This may be "
                     "expected\n if no path exists.)"
                  << std::endl
                  << std::endl;
        return std::list<PuzzleState>();
