...
// Helper function that can either print a warning message or throw an
// exception,
...
int main()
{
    ...
    // (See also the comment about randomization libraries in
    // GraphSearchCommon.h)
    srand(time(0));
...
void informalTests()
{
    ...
    // explanations. (See also the actual code in this file and the instructions
    // PDF.)
    ...
    if (showWelcome) {
        ...
        std::cout
            << "Make sure to inspect informalTests() in main.cpp to turn on "
               "the available"
            << std::endl
            << "informal tests. The terminal output for each test is lengthy, "
               "so you may"
            << std::endl
            << "want to only activate one test at a time." << std::endl
            << std::endl
            << "If the diagram plots in the terminal cause problems with "
               "screen readers,"
            << std::endl
            << "then please adjust the verbosity settings in the informalTests "
               "function."
            << std::endl
            << std::endl
            << "Remember that the informal tests are to help you debug with "
               "detailed text"
            << std::endl
            << "output. They are not the same tests used to grade your code. "
               "You can also"
            << std::endl
            << "run unit tests on your code that are more similar to our "
               "autograder."
            << std::endl
            << "Those are defined in the file(s) in the tests/ subdirectory. "
               "Since those"
            << std::endl
            << "other unit tests are separate from the main program, you must "
               "compile and"
            << std::endl
    ...
    if (showDemonstrations) {
        ...
        std::cout << makeHeader2("Skipping demonstrations.") << std::endl
                  << std::endl;
        std::cout << "  (Please do try the demonstrations and look at the "
                     "instructions PDF.)"
                  << std::endl
                  << std::endl;
    }
    ...
    if (doEx1Tests) {
        ...
        std::cout << makeHeader2("Skipping informal tests for Exercise 1.")
                  << std::endl
                  << std::endl;
    }
    ...
    if (doEx2Tests) {
        ...
        std::cout << makeHeader2("Skipping informal tests for Exercise 2.")
                  << std::endl
                  << std::endl;
    }
    ...
    if (doEx3Tests) {
        ...
        std::cout << "Now running puzzleBFS tests. Some tests may be disabled "
                     "for speed; see"
                  << std::endl
                  << "informalTests() in main.cpp for details. If any tests "
                     "take a very long time,"
                  << std::endl
                  << "you probably have a bug. You can cancel the running "
        ...
        // When your solution is correct, the ones we've provided run pretty
        // fast.

        // puzzleBFS edge cases: --------------------
        if (!doEx3SlowTests)
            std::cout << "(informalTests: doEx3SlowTests==false, so we'll skip "
                         "some slow tests.)"
                      << std::endl
                      << std::endl;

        if (doEx3SlowTests)
            puzzleBFS_Tester_NoPath(); // This one is slow even when your code
                                       // is correct!
        ...
        std::cout << makeHeader2("Skipping informal tests for Exercise 3.")
                  << std::endl
                  << std::endl;
    }
    ...
    std::cout
        ...
        << "\nIf any tests were disabled, you can enable them in the "
           "informalTests"
...
void demoRunner()
{

    {
        ...
        std::cout << "In this project, we use std::unordered_set in several "
                     "places to represent"
                  << std::endl
                  << "collections of items having some shared property. A set "
                     "contains value copies"
                  << std::endl
                  << "of objects, and it contains no duplicates; inserting the "
                     "same item twice has"
                  << std::endl
                  << "no effect. In C++, the unordered set type uses hashing "
                     "internally, so the"
                  << std::endl
                  << "key type must support equality comparison and std::hash. "
                     "Unlike the unordered"
                  << std::endl
                  << "map type, the keys for an unordered set do not have "
                     "associated values. We only"
                  << std::endl
                  << "care whether a given key has been inserted or not."
                  << std::endl
                  << std::endl
                  << "This is an alternative to \"labeling\" vertices with "
                     "some status, for example."
                  << std::endl
                  << "If an algorithm tells you to label vertices as "
                     "\"visited\", you could instead"
                  << std::endl
                  << "make a set for all the visited vertices, and insert "
                     "copies of vertices to the"
                  << std::endl
                  << "visited set when you want to record that they have been "
                     "visited."
                  << std::endl
                  << std::endl
                  << "For example, we could make a set to contain unique "
                     "integers like this:"
                  << std::endl
                  << std::endl
        ...
        std::cout << "size() is now: " << favoriteNumbers.size() << std::endl;
        std::cout << "Inserting 42 again. (This doesn't change anything.)"
                  << std::endl;
        favoriteNumbers.insert(42);
        std::cout << "size() is now: " << favoriteNumbers.size() << std::endl;

        std::cout << "At this point, the set contains 7 and 42. (It only has a "
                     "single copy of 42.)"
                  << std::endl
                  << std::endl;

        std::cout << "Iterating over std::unordered_set contents. (These may "
                     "not be in any order.)"
                  << std::endl;
        ...
        std::cout << "And then we can use count() to query if an item is in "
                     "the set or not, and we"
                  << std::endl
                  << "can remove items from the set with erase(). Remeber that "
                     "since a set contains"
                  << std::endl
                  << "unique items, we only ever expect a count of 1 or 0."
                  << std::endl
                  << std::endl;

        // Side note: Don't erase or insert items directly within a std::cout
        // statement, even if you want to display the returned value from the
        // operation. The order of evaluation for the stream syntax may not be
        // what you expect.
        std::cout << "Counting the key 7 (expecting 1): "
                  << favoriteNumbers.count(7) << std::endl;
        std::cout << "Erasing 7." << std::endl;
        favoriteNumbers.erase(7);
        std::cout << "Counting the key 7 (expecting 0): "
                  << favoriteNumbers.count(7) << std::endl
                  << std::endl;
    }

    {
        std::cout << makeHeader2(
                         "Undirected edge representation with IntPairPair")
                  << std::endl
                  << std::endl;

        std::cout << "Our GridGraph class doesn't explicitly store edges since "
                     "it uses an adjacency"
                  << std::endl
                  << "list scheme (actually, an adjacency map and set scheme "
                     "in our implementation)."
                  << std::endl
                  << "However, sometimes we may want to examine the undirected "
                     "edges of the graph"
                  << std::endl
                  << "as explcit items. For example, GridGraph::printDetails "
                     "makes a set of unique"
                  << std::endl
                  << "edges in order to display terminal output." << std::endl
                  << std::endl;

        std::cout << "This is where we have to be careful, because IntPairPair "
                     "is just a pair of"
                  << std::endl
                  << "pairs, so it doesn't guarantee the (A,B)==(B,A) equality "
        ...
        std::cout << "Creating IntPairPair AB from {pointA, pointB} and BA "
                     "from {pointB, pointA}."
                  << std::endl;
        IntPairPair AB = {pointA, pointB};
        IntPairPair BA = {pointB, pointA};
        std::cout << "Now is (AB == BA)? " << ((AB == BA) ? "true" : "false")
                  << std::endl;
        ...
        std::cout
            << "If you wanted to extend the code further, you could make a "
               "derived class"
            << std::endl
            << "based on IntPairPair that would ensure that edges (A,B) and "
               "(B,A) would"
            << std::endl
            << "automatically be tested equal and identically hashed. For this"
            << std::endl
            << "assignment, the provided code does that manually in a few "
               "places,"
            << std::endl
    ...
    {
        std::cout << makeHeader2("Various ways to initialize objects")
                  << std::endl
                  << std::endl;

        std::cout << "As you examine the provided code, you may see objects "
                     "being initialized in"
                  << std::endl
                  << "various ways. Remember that in C++11 and newer, there "
                     "are compiler features"
                  << std::endl
                  << "that can infer (detect) the correct STL types "
                     "automatically, which can help"
                  << std::endl
                  << "you write neater code." << std::endl
                  << std::endl
                  << "Our IntPair type is based on std::pair<int,int>. The "
                     "lines below all construct"
                  << std::endl
                  << "the same IntPair. The last one uses an \"initializer "
                     "list\" in braces {}, which"
                  << std::endl
                  << "can be used wherever the type can be understood "
        ...
        std::cout << "Displaying print output for all four objects:"
                  << std::endl
        ...
        std::cout << "However, if you're not explicit enough, the compilation "
                     "may fail."
                  << std::endl
                  << "The compiler needs to be able to tell if you mean "
                     "IntPair, std::vector,"
                  << std::endl
                  << "or something else, based on the structure of the "
                     "variable and how you use it."
                  << std::endl
                  << "The following may or may not work, depending on the "
...
void removeEdgeTester()
{

    {
        ...
        std::cout
            << "(Note: This function was probably implemented for you already.)"
            << std::endl
            << std::endl;
        ...
        if (graph2 == expectedGraph) {
            ...
            throw std::runtime_error(
                "The starter code did not remove the edge correctly!");
        }
    }
}
...
void removePointTester()
{
    ...
    for (const auto &kv : graph.adjacencyMap) {
        ...
        if (k == IntPair(2, 2)) {
            ...
            // the value type is a set of neighboring points that are adjacent
            // to this point in the graph
            const GridGraph::NeighborSet &adjacencies = kv.second;
            for (const IntPair &neighbor : adjacencies) {
                if (neighbor == IntPair(2, 2)) {
                    errorReaction(
                        "Found (2,2) listed as an adjacent point somewhere");
                }
            }
        }
    }
    ...
    if (graph == edgelessGraph) {
        std::cout << "Good, it looks like the point removal worked correctly."
                  << std::endl
                  << std::endl;
    }
    else {
        errorReaction("The point removal should only remove the point\n and "
                      "all the adjacency records that refer to it.");
    }

    GridGraph backupGraph = graph;

    std::cout
        << "Now trying to remove a non-existent point (7,7)..." << std::endl
        << "The graph should be unchanged and the program shouldn't crash."
        << std::endl
        << std::endl;
    ...
    if (graph == backupGraph) {
        std::cout << "Good, the graph was unchanged by that operation."
                  << std::endl
                  << std::endl;
    }
}
...
void graphBFS_Tester()
{
    ...
    std::cout << makeHeader2("Testing graphBFS (when a path does exist)")
              << std::endl
              << std::endl;

    std::cout << "A path from start to goal does exist in this graph, and the "
                 "edges all have\n"
              << "the same length, so your graphBFS function should be able to "
                 "find and return\n"
    ...
    // Remove certain edges to leave a maze. (These coordinates were figured out
    // beforehand.) Note that for convenience, we can use the C++11
    // "initializer" syntax in curly braces {} to concisely enter all these
    // nested objects. As long as the compiler can find constructors that match
    // the types we're using, it will be okay.
    std::vector<IntPairPair> edgesToRemove = {
        {{0, 1}, {0, 2}}, {{0, 3}, {1, 3}}, {{1, 1}, {2, 1}}, {{1, 2}, {1, 3}},
        {{1, 3}, {2, 3}}, {{1, 4}, {2, 4}}, {{2, 0}, {2, 1}}, {{2, 1}, {2, 2}},
        {{2, 2}, {2, 3}}, {{3, 0}, {3, 1}}, {{3, 2}, {3, 3}}, {{3, 2}, {4, 2}},
        {{3, 3}, {4, 3}}, {{3, 4}, {3, 5}}, {{4, 0}, {4, 1}}, {{4, 1}, {4, 2}},
        {{4, 3}, {5, 3}}, {{4, 4}, {4, 5}}, {{4, 4}, {5, 4}}, {{4, 5}, {5, 5}},
    ...
    if (path.empty()) {
        errorReaction("Could not reach goal from start. BFS function may be "
                      "incomplete or buggy.");
    }
    else {
        ...
        if (path.front() != maze_start) {
            allGood = false;
            errorReaction(
                "First point in reconstructed path was not start.\n Make sure "
                "you've set predecessors correctly with pred[].\n (And make "
                "sure you don't have it backwards.)");
        }
        if (path.back() != maze_goal) {
            allGood = false;
            errorReaction(
                "Last point in reconstructed path was not goal.\n Make sure "
                "you've set predecessors correctly with pred[].\n (And make "
                "sure you don't have it backwards.)");
        }
        ...
        if (!GridGraph::allowVerboseTextDescription && !forceDescribePath) {
            std::cout << "Omitting verbose text description of path. You can "
                         "change this in main.cpp"
                      << std::endl
                      << " in the graphBFS_Tester function definition."
                      << std::endl
                      << std::endl;
        }
        ...
        while (!working_path.empty()) {
            ...
            if (!graph.hasEdge(p1, p2)) {
                allGood = false;
                std::cout << "Reported edge: " << p1 << " to " << p2
                          << std::endl;
                errorReaction("The reported path edge doesn't exist in the "
                              "original graph");
            }
            else {
                // When plotting is not allowed, we take special care to print
                // the verbose details of the path in path order.
                if (forceDescribePath ||
                    (!GridGraph::allowPlotting &&
                     GridGraph::allowVerboseTextDescription)) {
                    std::cout << "[(" << p1.first << "," << p1.second
                              << ") to (" << p2.first << "," << p2.second
                              << ")] ";
                }
            }
            // step forward
        ...
        // The default behavior of << when plotting is not allowed would be to
        // print details of the path edges all jumbled out of order.
        ...
        if (num_steps != 19) {
            allGood = false;
            errorReaction(
                "The path returned is not the shortest possible path");
        }

        std::cout << std::endl;

        if (allGood)
            std::cout << "Good! You found the shortest path to the goal."
                      << std::endl;
        else
            std::cout << "Some warnings or errors occurred in this test."
                      << std::endl;
    }
...
void graphBFS_Tester_NoPath()
{
    ...
    std::cout << makeHeader2("Testing graphBFS (when there is no path)")
              << std::endl
              << std::endl;

    std::cout
        << "In this maze there's no way to reach the goal from the start,\n"
    ...
    std::vector<IntPairPair> edgesToRemove = {
        ...
        {{6, 2}, {6, 3}},
        {{6, 4}, {6, 5}},
        // Removing this last edge separates the graph into two connected
        // components, where the start no longer has a path to the goal:
        {{0, 2}, {0, 3}}};
    ...
    std::list<IntPair> path = graphBFS(maze_start, maze_goal, graph);
    if (path.empty()) {
        std::cout << "Good, an empty path was returned in this case."
                  << std::endl;
    }
    else {
        std::cout << "Error: Your graphBFS returned some path when none should "
                     "exist.\n"
                  << "Here is the path that your graphBFS returned:"
                  << std::endl
                  << std::endl;
        ...
        errorReaction(
            "graphBFS returned some path when no path should be found");
    }
...
void graphBFS_Tester_StartAtGoal()
{
    ...
    if (path.empty()) {
        errorReaction(
            "When starting at the goal, graphBFS should return a "
            "path\ncontaining only that one point, but not an empty path.");
    }
    else {
        std::cout << "Here is the path that your graphBFS returned:"
                  << std::endl
                  << "(It should be a single point (0,0), in this case.)"
                  << std::endl
                  << std::endl;
        ...
        if (allGood)
            std::cout << "Good! When start==goal, the shortest path is to stay "
                         "at the start."
                      << std::endl;
        else
            std::cout << "Some warnings or errors occurred in this test."
...
void puzzleBFS_Tester()
{
    ...
    std::cout << makeHeader2("Testing puzzleBFS (simple puzzle instance)")
              << std::endl
              << std::endl;

    std::cout
        << "This puzzle can be solved from the given starting position, so "
           "puzzleBFS"
        << std::endl
        << "should be able to solve it and find the shortest solution too."
        << std::endl
        << std::endl;
    ...
    // Print a compact string version of the state as well as the plot diagram
    // (if allowed):
    ...
    if (path.empty()) {
        ...
        std::cout
            << "Returned some path (list of sequential states). Validating..."
            << std::endl
            << std::endl;
        ...
        if (path.front() != puzzle_start) {
            allGood = false;
            errorReaction(
                "First state in reconstructed sequence was not start.\n Make "
                "sure you've set predecessors correctly with pred[].\n (And "
                "make sure you don't have it backwards.)");
        }
        if (path.back() != puzzle_goal) {
            allGood = false;
            errorReaction(
                "Last state in reconstructed sequence was not goal.\n Make "
                "sure you've set predecessors correctly with pred[].\n (And "
                "make sure you don't have it backwards.)");
        }
        ...
        if (allGood)
            std::cout << "Good! You found the shortest solution sequence."
                      << std::endl;
        else
            std::cout << "Some warnings or errors occurred in this test."
                      << std::endl;
...
void puzzleBFS_Tester_NoPath()
{
    ...
    std::cout << makeHeader2("Testing puzzleBFS (when there is no solution)")
              << std::endl
              << std::endl;

    std::cout << "(Please allow at least 10 seconds for this test to complete. "
                 "If this is"
              << std::endl
              << " inconvenient, you can disable this test in the "
                 "informalTests function"
              << std::endl
              << " in main.cpp.)" << std::endl
              << std::endl;

    std::cout << "This puzzle can't be solved from the given starting state,"
              << std::endl
    ...
    if (path.empty()) {
        ...
        std::cout << "Error: Your puzzleBFS returned some path when none "
                     "should exist.\n"
                  << "Here is the path that your puzzleBFS returned:"
                  << std::endl
                  << std::endl;
        ...
        errorReaction(
            "puzzleBFS returned some path when no path should be found");
    }
...
void puzzleBFS_Tester_StartAtGoal()
{
    ...
    std::cout << makeHeader2("Testing puzzleBFS (when start==goal)")
              << std::endl
              << std::endl;
    ...
    std::list<PuzzleState> path = puzzleBFS(puzzle_start, puzzle_goal);
    if (path.empty()) {
        errorReaction("Returned an empty path, but should have returned a path "
                      "containing only start.");
    }
    else {
        std::cout
            << "Returned some path (list of sequential states). Validating..."
...
void puzzleBFS_Tester_Random()
{
    ...
    std::cout << makeHeader2("Testing puzzleBFS with a random, solvable puzzle")
              << std::endl
              << std::endl;
    ...
    const PuzzleState puzzle_start =
        PuzzleState::randomizePuzzle(puzzle_goal, max_steps);
