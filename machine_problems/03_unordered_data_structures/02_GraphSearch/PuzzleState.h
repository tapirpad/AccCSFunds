...
public:
    ...
    PuzzleState(const PuzzleState &other) : data(other.data)
    {
        if (!isValid()) {
            throw std::runtime_error(
                "PuzzleState copy constructor: invalid other");
        }
    }
    ...
    PuzzleState &operator=(const PuzzleState &other)
    {
        if (!other.isValid()) {
            throw std::runtime_error(
                "PuzzleState copy assignment: invalid other");
        }
        data = other.data;
        return *this;
    }
    ...
    // the entire class, not just one instance. Outside of the class
    // declaration, it could be called by users with
    // "PuzzleState::validArray(...)".
    static bool validateArray(const std::array<int, 9> &data)
    {
        ...
        if (dataCopy != std::array<int, 9>({1, 2, 3, 4, 5, 6, 7, 8, 9})) {
            std::cout << "validateArray warning: data must contain a "
                         "permutation of numbers 1 through 9"
                      << std::endl;
            return false;
        }
        return true;
    ...
    // [1,2,3,4,5,6,7,8,9] Can be used along with normal std::cout operations
    ...
    // We conceptualize tile moves as moving the "blank" space through the
    // puzzle, although realistically, we would actually move the tile next to
    // the blank space into that space (in the opposite direction). So for the
    // sake of our code, we will think of moving the blank space (the 9 digit)
    // around.
    ...
    int getBlankIndex() const
    {
        ...
        throw std::runtime_error(
            "getBlankIndex: not found; invalid puzzle state");
    }
    ...
    bool getUpState(PuzzleState &outputState) const
    {
        ...
        // Replace the passed PuzzleState with a fresh working copy of the
        // current state.
        outputState = *this;
        ...
        // The outputState has been updated in place, so we don't have to pass
        // it back. Return true since the state was found.
        return true;
    }
    ...
    std::vector<PuzzleState> getAdjacentStates() const
    {
        ...
        // Try "moving" the blank in all four directions. If the move is
        // possible, then the tempState argument will be updated with the new
        // state, which we then store in the adjacentStates vector.
    ...
    // That is, it returns a random adjacent state to the current state. We
    // avoid duplicating the previous state. This can be used to take random
    // steps and generate a puzzle for testing. If there is no previous state,
    // or if you don't care about possibly undoing the most recent step, then
    // just pass the current state as both the current and previous state.
